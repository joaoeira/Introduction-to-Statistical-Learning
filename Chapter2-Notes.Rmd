...
This equation tells us that in order to minimize the expected test error, we need to select a statistical learning method that simultaneously achieves *low variance* and *low bias*. Not that variance and square bias are nonnegative quantities, hence the expected MSE can never lie below $Var(\epsilon)$, the irreducible error.
...
Where $I(y_i \neq \hat{y}_i$ is an *indicator variable* that equals 1 if they are not equal, and zero if they are. Hence the euqation computes the incorrect classifications.
